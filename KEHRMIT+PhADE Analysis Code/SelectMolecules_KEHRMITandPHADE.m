...
% This script is used to select replication bubbles from KEHRMIT and/or PHADE/CIDER movies 
% PHADE and CIDER movies are treated exactly the same
%
% Before running this function you need to set the RawDataPath and the AnalysisPath
% >> SetRawDataPath
% >> SetAnalysisPath
%
% You can run this function in one of three modes:
%   SelectMolecules_KEHRMITandPHADE('KEHRMIT')       for KEHRMIT only data
%   SelectMolecules_KEHRMITandPHADE('PHADE')         for PHADE/CIDER only data
...
CLimKehrmit = [500 3000];
CLimPhade   = [1000 9000];

%% Filtering Parameters for Background Subtraction and for Smoothing
BackgrSubtrFilter  = 30; %this is used to remove uneven background. 20-50px filtering is optimal
MaxProjGaussFilter = 2; %this is used for generating the maximum projection only - since it's susceptible to noise, the raw data is pre-filtered before making the MaxProj
...
%% Window/GUI dimensions
    ...
    Tpanel = uipanel('Title','Shortcuts','TitlePosition','centertop','FontSize',10,'Units','Normalized',...
                     'Position',[0.8678    0.29   0.1252    0.3]);
