...
% CustomText = [FileName ', Molecule #' num2str(CurrMol) '/' num2str(TotalMol)]; %display the filename in the title of the figure
% KymoSize = size(Composite);
% Composite = insertText(Composite,[2 KymoSize(1)-15],['F' FileName(end-1:end) 'M' num2str(i)],'FontSize',8,'TextColor','white','BoxOpacity',0);
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   
%         case 'd' %Measure Distance on the image (to assess HpaII separation)
%             [x1, y1] = SelectKymoROI_KEHRMITandPHADE_Ginputc(1,'Color','g'); hold on; h1=plot(x1,y1,'+g');
%             [x2, y2] = SelectKymoROI_KEHRMITandPHADE_Ginputc(1,'Color','g'); hold on; h2=plot(x2,y2,'+g'); delete(h1); delete(h2);
%             
%             % get the parameters for the current experiment
%             temp = load('AnalysisPath.mat'); AnalysisPath = temp.AnalysisPath; clear temp;
%             ParamsFile = [AnalysisPath filesep 'AnalysisParams.txt'];
%             fileID = fopen(ParamsFile);
%             Line1 = fgetl(fileID); temp = textscan(Line1,'%f'); PixelSize = temp{1};
%             Line2 = fgetl(fileID); temp = textscan(Line2,'%f'); DeltaT    = temp{1};
%             Line3 = fgetl(fileID); temp = textscan(Line3,'%f'); Stretch   = temp{1};
%             Line4 = fgetl(fileID); temp = textscan(Line4,'%f'); NmToBp    = temp{1};
%             fclose(fileID);
%             DistPX = abs(diff([x1 x2])); %in pixels
%             DistKBP = DistPX*PixelSize/Stretch/NmToBp/1000; %in kbp
%             msgbox([num2str(DistKBP) ' kbp']);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        case 'e' %save
            UserData   = get(gcf,'UserData');
