...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
    ...
    % To save space on the drive and simplify backups, individual kymograms will not be saved.
    %             KymoDir = [FolderName filesep FileName '_KymoKaP']; %folder where the individual Kymogram images are saved
    %             if ~isfolder(KymoDir)
    %                 mkdir(KymoDir);
    %             end
    %             
    %             %Save screenshot of the selection window highlighting all selections
    %             saveas(gcf,[FolderName filesep FileName '_ScreenShot.png']);
    %             
    %             WaitBarH = waitbar(0,'Saving your molecule selections'); pause(.5); %display progress
    %             for i=1:length(UserData.SelectX) %generate all the kymograms and save them
    %                 if strcmp(UserData.Mode,'KEHRMIT')
    %                     CmgKymogram = mat2gray(UserData.Kymogram{i},UserData.CLim);    %apply the Black/White Point
    %                     RgbKymogram = cat(3, CmgKymogram, CmgKymogram, CmgKymogram);   %rgbImage = cat(3, redChannel, greenChannel, blueChannel);                    
    %                 elseif strcmp(UserData.Mode,'PHADE')
    %                     FenKymogram = mat2gray(UserData.Kymogram2{i},UserData.CLim2);  %apply the Black/White Point
    %                     RgbKymogram = cat(3, FenKymogram, FenKymogram, FenKymogram);   %rgbImage = cat(3, redChannel, greenChannel, blueChannel);
    %                 elseif strcmp(UserData.Mode,'KEHRMIT+PHADE')
    %                     CmgKymogram = mat2gray(UserData.Kymogram{i},UserData.CLim);    %apply the Black/White Point
    %                     FenKymogram = mat2gray(UserData.Kymogram2{i},UserData.CLim2);  %apply the Black/White Point
    %                     RgbKymogram = cat(3, CmgKymogram*0, CmgKymogram, FenKymogram); %rgbImage = cat(3, redChannel, greenChannel, blueChannel);
    %                 end
    %                                         
    %                 %Add text to the bottom
    %                 KymoSize = size(RgbKymogram);
    %                 RgbKymogram = insertText(RgbKymogram,[2 KymoSize(1)-15],['F' FileName(end-1:end) 'M' num2str(i)],'FontSize',8,'TextColor','white','BoxOpacity',0);
    %                 imwrite(RgbKymogram,[KymoDir filesep 'KymoKaP_' num2str(i) '.png']);
    %                 waitbar(i/length(UserData.SelectX),WaitBarH,'Saving kymograms');
    %             end
    %             
    %             close(WaitBarH);
    end
